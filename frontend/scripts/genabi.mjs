...
const CONTRACT_NAME = "EnergyLogStorage";
...
// Check if ABI files already exist (for Vercel/CI environments)
...
// If both ABI files exist and deployments directory doesn't exist, skip generation
// This is useful for Vercel deployments where deployments directory is not available
if (!fs.existsSync(deploymentsDir) && fs.existsSync(existingABIFile) && fs.existsSync(existingAddressesFile)) {
  console.log(`[genabi] Deployments directory not found, but ABI files already exist. Skipping generation.`);
  console.log(`[genabi] This is normal for Vercel/CI deployments where ABI files are pre-generated.`);
  process.exit(0);
}
